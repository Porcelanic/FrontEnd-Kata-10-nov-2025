...
export default function MisSolicitudesPage() {
  ...
  const loadSolicitudes = () => {
    ...
    const misSolicitudes = todasSolicitudes.filter((s) => s.solicitante_id === user?.id)
    setSolicitudes(misSolicitudes)
  }

  const solicitudesFiltradas =
    filtroTipo === "todas" ? solicitudes : solicitudes.filter((s) => s.tipo_solicitud === filtroTipo)
      ...
      <main className="flex-1 lg:ml-64 p-8">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Mis Solicitudes</h1>
          ...
          {solicitudesFiltradas.length === 0 ? (
            <Card className="dark:bg-slate-800 dark:border-slate-700">
              <CardContent className="pt-6">
                <p className="text-center text-gray-500 dark:text-slate-400">No tienes solicitudes</p>
              </CardContent>
            </Card>
          ) : (
